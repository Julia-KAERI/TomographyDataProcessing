...
@eq-fourier_transform_of_slice_4 의 $|\rho|$ 는 high-pass filter 역할을 하며(이를 *Lamp 필터* 혹은 *Ram-Lack 필터* 라고 한다) 이로인해 high frequency 영역에서의 노이즈에 매우 취약하게 된다. 이로인해 $Q_\theta (t)$ 를 계산할 때 window function 이라고 불리우는 low pass filter 를 곱하여 푸리에 변환을 수행한다. 각 필터의 모양은 아래 @fig-fbp_window 와 같다.

![Window functions](figures/fourier_filter.png){#fig-fbp_window width=600}
